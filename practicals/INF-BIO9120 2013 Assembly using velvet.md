...
* understand the significance the mst important setting for Velvet 
...
We will assemble *E. coli K12 *strain MG1655 which was sequenced on an Illumina MiSeq. The sequencing centre read 150 bases from each direction.
...
Velvet requires an index file to be built before the assembly takes place. We must choose a *k-*mer value for building the indes. Longer *k-*mers result in a more stringent assembly, at the expense of coverage. There is no definitive value of *k* for any given project. However, there are several absolute rules:
...
* Convert this value from k-mer coverage into genome coverage (average number of the genome was sequenced).
...
**For this exercise**, use the the knwon length for this strain, 4.6 Mbp, for the genome size
...
As both the mate pairs and the paired end reads are of the same length, and privide the same coverage, it could be intersting to try an assembly of the mate pair reads only. The read sequences would still be used to build the contigs, and the mate pair information to build scaffolds.
